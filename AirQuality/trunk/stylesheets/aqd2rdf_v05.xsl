...
<xsl:stylesheet version="1.0"
        ...
        xmlns:ef="http://inspire.ec.europa.eu/schemas/ef/3.0rc2"
...
<!-- Empty wrappers -->
  <xsl:template match="gml:featureMember|aqd:content">
    <xsl:apply-templates mode="resource"/>
  </xsl:template>

  <xsl:template match="aqd:AQD_ReportingHeader" mode="resource">
...
<!-- Stop processing of properties -->
  <xsl:template match="ef:geometry|am:geometry|sams:shape|om:result|aqd:inspireId" mode="property"/>

<!-- Generate link to Inspire Id -->
  <xsl:template match="ef:inspireId" mode="property">
    <forInspireId><xsl:attribute name="rdf:resource"><xsl:value-of select="base:Identifier/base:namespace"/><xsl:value-of select="base:Identifier/base:localId"/></xsl:attribute>
    </forInspireId>
  </xsl:template>
