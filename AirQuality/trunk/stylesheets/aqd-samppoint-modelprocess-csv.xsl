...
<!-- $Id$
    AQD_ModelProcess for models & objective estimation business data 2 - model & objective estimation processes / configurations
...
<xsl:template match="gml:FeatureCollection">
  ...
  <xsl:call-template name="table">
    <xsl:with-param name="nodetype" select="gml:featureMember/aqd:AQD_ModelProcess"/>
  </xsl:call-template>
</xsl:template>

<xsl:template match="aqd:content">
  <xsl:call-template name="table">
    <xsl:with-param name="nodetype" select="aqd:AQD_ModelProcess"/>
  </xsl:call-template>
</xsl:template>
...
<xsl:template name="header">
    ...
    <xsl:text>TemporalResolutionUnit</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>TemporalResolutionNum</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>SpatialResolution</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>DataQualityDescription</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>DataQualityReport</xsl:text><xsl:text>&nl;</xsl:text>
</xsl:template>
...
<xsl:template name="row">
    ...
    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="ompr:inspireld/base:Identifier/base:localId"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="ompr:inspireld/base:Identifier/base:namespace"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="ompr:inspireld/base:Identifier/base:versionId"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>
    ...
    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:description"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:temporalResolution/aqd:TimeReferences/aqd:unit/@xlink:href"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:temporalResolution/aqd:TimeReferences/aqd:numUnits"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:spatialResolution"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:dataQualityDescription"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:dataQualityReport"/>
    </xsl:call-template>
