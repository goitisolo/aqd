...
<!-- $Id$
    AQD_Network for fixed measurements business data 8 - General network information
...
<xsl:template match="gml:FeatureCollection">
  ...
  <xsl:call-template name="table">
    <xsl:with-param name="nodetype" select="gml:featureMember/aqd:AQD_Network"/>
  </xsl:call-template>
</xsl:template>

<xsl:template match="aqd:content">
  <xsl:call-template name="table">
    <xsl:with-param name="nodetype" select="aqd:AQD_Network"/>
  </xsl:call-template>
</xsl:template>
...
<xsl:template name="header">
    ...
    <xsl:text>NetworkType</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>OperationBegin</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>OperationEnd</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>Timezone</xsl:text><xsl:text>&sep;</xsl:text>
    <xsl:text>ResponsibleParty</xsl:text><xsl:text>&nl;</xsl:text>
</xsl:template>
...
<xsl:template name="row">
    ...
    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:networkType/@xlink:href"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:operationActivityPeriod/gml:TimePeriod/gml:beginPosition"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:operationActivityPeriod/gml:TimePeriod/gml:endPosition"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="aqd:aggregationTimeZone/@xlink:href"/>
    </xsl:call-template>
    <xsl:text>&sep;</xsl:text>

    <xsl:call-template name="wrapext">
      <xsl:with-param name="value" select="ef:responsibleParty"/>
    </xsl:call-template>
