...
let $resultTime := xs:dateTime($asessments/sparql:binding[@name = "resultTime"]/sparql:literal)
let $assessmentMethodSparql:=($asessments/sparql:binding[@name = "assessmentMethod"]/sparql:literal)
let $getAssessmentMethodsAndDates_query := sparqlx:getLink(query:getAssessmentMethodsAndDates($e1aUrl,$reportingYear,$regex,$time1,$time2))
let $XMlResultTime := min(for $x at $xpos in $docRoot//om:OM_Observation/om:resultTime/gml:TimeInstant/gml:timePosition
